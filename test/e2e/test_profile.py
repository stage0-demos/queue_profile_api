...
def test_create_profile_endpoint():
    """Test POST /api/profile endpoint and verify record persists in database."""
    ...
    data = {
        ...
        "description": "E2E test profile document",
    }

    response = requests.post(f"{BASE_URL}/api/profile", headers=headers, json=data)
    assert response.status_code == 201, _err(response, 201)

    response_data = response.json()
    assert "_id" in response_data, "Response missing '_id' key"
    assert response_data["name"] == "e2e-test-profile"
...
def test_get_profiles_with_name_filter():
    ...
    headers = {"Authorization": f"Bearer {token}"}
    response = requests.get(f"{BASE_URL}/api/profile?name=e2e", headers=headers)
    assert response.status_code == 200, _err(response, 200)
