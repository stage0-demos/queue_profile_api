"""
E2E tests for Organization endpoints.

These tests verify that Organization endpoints work correctly by making
...
def test_create_organization_endpoint():
    """Test POST /api/organization endpoint and verify record persists in database."""
    ...
    data = {
        ...
        "description": "E2E test organization document",
    }

    response = requests.post(f"{BASE_URL}/api/organization", headers=headers, json=data)
    assert response.status_code == 201, _err(response, 201)

    response_data = response.json()
    assert "_id" in response_data, "Response missing '_id' key"
    assert response_data["name"] == "e2e-test-organization"
    assert "created" in response_data
    assert "saved" in response_data


@pytest.mark.e2e
def test_get_organizations_endpoint():
...
def test_get_organizations_with_name_filter():
    ...
    headers = {"Authorization": f"Bearer {token}"}
    response = requests.get(f"{BASE_URL}/api/organization?name=e2e", headers=headers)
    assert response.status_code == 200, _err(response, 200)
